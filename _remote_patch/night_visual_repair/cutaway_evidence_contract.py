#!/usr/bin/env python3
"""Validate hash-bound, five-view cutaway room-render evidence without Blender."""
...
def _verify_bound_file(record: Any, *, expected_path: Path, label: str) -> list[str]:
    ...
    resolved, expected = path.resolve(), expected_path.resolve()
    failures: list[str] = []
    if resolved != expected:
        return [f"{label}: path differs from canonical file; recorded={resolved}, expected={expected}"]
    if not resolved.is_file() or resolved.stat().st_size < 1:
        return [f"{label}: file is missing or empty: {resolved}"]
    size_value = record.get("size_bytes") if "size_bytes" in record else record.get("image_size_bytes")
    if size_value != resolved.stat().st_size:
        failures.append(f"{label}: recorded size differs from current file")
    digest_value = record.get("sha256") if "sha256" in record else record.get("image_sha256")
...
def validate_cutaway_evidence(
    ...
    *,
    ...
    if len(canonical_images) != len(VIEW_NAMES):
        failures.append("cutaway evidence requires exactly five canonical images")
    elif [path.name for path in canonical_images] != expected_names:
        failures.append("cutaway image names/order differ from the canonical top, oblique, and detail views")
    ...
    if evidence.get("expected_views") != list(VIEW_NAMES):
        failures.append("cutaway expected_views are not the exact five-view contract")
    if evidence.get("rendered_view_count") != len(VIEW_NAMES):
        failures.append("cutaway rendered_view_count is not exactly five")
    failures.extend(_verify_bound_file(evidence.get("source_blend"), expected_path=source_blend, label="cutaway/source_blend"))
    ...
    if not isinstance(views, list) or len(views) != len(VIEW_NAMES):
        failures.append("cutaway evidence does not contain exactly five views")
        views = []
    if [view.get("view_name") for view in views if isinstance(view, dict)] != list(VIEW_NAMES):
    ...
    for index, view_name in enumerate(VIEW_NAMES):
        ...
        if index < len(canonical_images):
            failures.extend(_verify_bound_file(view, expected_path=canonical_images[index], label=f"cutaway/views/{view_name}/image"))
        ...
        overhead_state, hidden_overhead = cutaway.get("overhead_state"), cutaway.get("hidden_overhead")
        ...
        if view_name == "top":
            ...
            failures.append(f"cutaway/views/{view_name}: no camera-side wall was hidden")
        for label in ("visible_far_wall_object_names", "visible_floor_object_names", "visible_content_object_names"):
            values = cutaway.get(label)
            if not isinstance(values, list) or not values:
                failures.append(f"cutaway/views/{view_name}: {label} is empty")
        if not isinstance(cutaway.get("classified_content_count"), int) or cutaway.get("classified_content_count", 0) < 1:
