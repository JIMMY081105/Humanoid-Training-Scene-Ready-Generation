...
class FurnitureSelection:
    ...
    def __post_init__(self) -> None:
        """Preserve explicit optionality when a plan is restored.

        A board/display selection with only optional visual dressing is not a
        required manipuland target, so a model with no usable support surface
        must be allowed to remain empty.  This covers both explicit
        no-requirement wording and wall displays with optional accessories.
        Append the canonical marker consumed by the checkpointed workflow
        without changing an existing required or cabinet/desk selection.
        """
        constraints = self.prompt_constraints.strip()
        has_required_item = "required:" in self.suggested_items.casefold()
        canonical_marker = "no specific manipulands required"
        furniture_id = str(self.furniture_id).casefold()
        is_optional_wall_display = any(
            token in furniture_id for token in ("board", "chart", "pocket")
        )
        has_explicit_no_requirement = (
            "no specific requirements for this surface" in constraints.casefold()
            or "no explicit item requirements" in constraints.casefold()
        )
        if (
            (has_explicit_no_requirement or is_optional_wall_display)
            and not has_required_item
            and canonical_marker not in constraints.casefold()
        ):
            self.prompt_constraints = (
                f"{constraints} No specific manipulands required."
            )
