...
class RoomScene:
    ...
    def generate_surface_id(self) -> UniqueID:
        ...
        """
        occupied = self._synchronize_surface_id_counter(reset=False)
        while True:
            suffix = _int_to_base36(self._surface_id_counter)
            surface_id = UniqueID(f"S_{suffix}")
            self._surface_id_counter += 1
            if surface_id not in occupied:
                return surface_id

    def _synchronize_surface_id_counter(self, *, reset: bool) -> set[UniqueID]:
        """Bind the allocator to every live support surface and reject aliases.

        Serialized checkpoints do not include ``_surface_id_counter``.  A restored
        scene may therefore already contain ``S_0`` while the in-memory allocator is
        back at zero.  Surface IDs are used as placement references, so silently
        reissuing one can associate a manipuland with the wrong furniture.

        Args:
            reset: Rebuild the counter exactly from the live scene when true.
                Otherwise only advance the existing counter.

        Returns:
            The complete occupied surface-ID set.

        Raises:
            RuntimeError: If two distinct live surfaces have the same ID.
        """
        owners: dict[UniqueID, UniqueID] = {}
        candidates = list(self.objects.values())
        floor = self.room_geometry.floor if self.room_geometry else None
        if floor is not None:
            object_alias = self.objects.get(floor.object_id)
            if object_alias is None:
                candidates.append(floor)
            elif object_alias is not floor:
                raise RuntimeError(
                    f"Room floor ID {floor.object_id} aliases a distinct scene object"
                )

        max_generated_index = -1
        for obj in candidates:
            for surface in obj.support_surfaces:
                surface_id = UniqueID(str(surface.surface_id))
                if surface_id in owners:
                    raise RuntimeError(
                        "Duplicate support surface ID "
                        f"{surface_id!s} on {owners[surface_id]!s} and "
                        f"{obj.object_id!s}"
                    )
                owners[surface_id] = obj.object_id
                raw_id = str(surface_id)
                if not raw_id.startswith("S_"):
                    continue
                suffix = raw_id[2:]
                try:
                    index = int(suffix, 36)
                    ...
                    continue
                if suffix and _int_to_base36(index) == suffix:
                    max_generated_index = max(max_generated_index, index)

        next_index = max_generated_index + 1
        if reset:
            self._surface_id_counter = next_index
        else:
            self._surface_id_counter = max(self._surface_id_counter, next_index)
        return set(owners)
...
- add_weld:
    ...
    def restore_from_state_dict(self, state_dict: dict[str, Any]) -> None:
        ...
        # Checkpoints intentionally serialize semantic scene state, not allocator
        # internals.  Rebuild the global surface allocator before any later target
        # extracts another surface.
        self._synchronize_surface_id_counter(reset=True)
...
def _recover_rolling_whiteboard_marker_trays(
    furniture_object: SceneObject,
    config: SupportSurfaceExtractionConfig | None,
) -> list[SupportSurface]:
    """Recover real marker-tray faces omitted by generic HSM filtering.

    The generated rolling-whiteboard asset used by the classroom contract has
    three shallow, upward-facing tray patches at its mid-height.  They are real
    mesh faces, but their 4--8 cm depth and immediate board backstop make them
    fail the generic 10 cm inscribed-radius and vertical-clearance filters.  A
    broad fallback would make arbitrary thin rails usable, so this recovery is
    deliberately limited to an explicitly rolling whiteboard and retains only
    wide, mid-height tray patches.  Normal placement collision validation still
    runs for every object placed on the recovered surfaces.
    """
    if config is None or furniture_object.geometry_path is None:
        return []

    name = str(furniture_object.name or "").casefold()
    description = str(furniture_object.description or "").casefold()
    if name != "whiteboard" or "rolling" not in description:
        return []

    relaxed_config = deepcopy(config)
    relaxed_config.min_clearance_m = 0.0
    relaxed_config.min_inscribed_radius_m = min(
        relaxed_config.min_inscribed_radius_m, 0.02
    )
    # The configured 0.1 m² minimum is appropriate for tables and shelves, but
    # a genuine marker tray is intentionally smaller (roughly 0.015--0.05 m²).
    # Width/depth and height checks below keep this relaxation from admitting
    # arbitrary small faces.
    relaxed_config.min_surface_area_m2 = min(
        relaxed_config.min_surface_area_m2, 0.003
    )
    try:
        candidates = extract_support_surfaces_from_mesh(
            mesh_path=furniture_object.geometry_path,
            config=relaxed_config,
        )
    except (OSError, ValueError) as exc:
        console_logger.warning(
            "Could not inspect rolling-whiteboard tray faces for %s: %s",
            furniture_object.object_id,
            exc,
        )
        return []

    heights = np.asarray(
        [surface.transform.translation()[2] for surface in candidates], dtype=float
    )
    finite_heights = heights[np.isfinite(heights)]
    if finite_heights.size < 2:
        return []
    min_height = float(np.min(finite_heights))
    height_span = float(np.max(finite_heights) - min_height)
    if height_span <= 0.0:
        return []

    trays: list[SupportSurface] = []
    for surface in candidates:
        height = float(surface.transform.translation()[2])
        normalized_height = (height - min_height) / height_span
        dimensions = surface.bounding_box_max - surface.bounding_box_min
        width = float(dimensions[0])
        depth = float(dimensions[1])
        if (
            not np.isfinite([normalized_height, width, depth]).all()
            or not 0.35 <= normalized_height <= 0.60
            or width < 0.18
            or depth < 0.035
        ):
            continue
        trays.append(surface)

    # The largest real tray face is the usable marker ledge.  The two small
    # sloped faces beside it are trim details: treating them as supports gives
    # small objects an apparently plausible but physically unstable resting
    # surface.  Use the broad ledge only, flatten it to the visual horizontal
    # plane, and bind it to the one-for-one repaired collision hull prepared
    # beside this generated asset.  The repair preserves the 32-hull limit.
    main_tray = max(
        trays,
        key=lambda item: float(item.bounding_box_max[0] - item.bounding_box_min[0]),
        default=None,
    )
    if main_tray is None:
        return []
    width = float(main_tray.bounding_box_max[0] - main_tray.bounding_box_min[0])
    if width < 0.75:
        return []
    repaired_tray = _repair_rolling_whiteboard_marker_tray(
        main_tray, furniture_object
    )
    if repaired_tray is None:
        return []
    trays = [repaired_tray]

    if trays:
        console_logger.info(
            "Recovered %d real marker-tray face(s) for rolling whiteboard %s",
            len(trays),
            furniture_object.object_id,
        )
    return trays
...
def _repair_rolling_whiteboard_marker_tray(
    surface: SupportSurface,
    furniture_object: SceneObject,
) -> SupportSurface | None:
    """Build the single level marker ledge and activate its matching hull.

    The generated asset's original sixth convex hull extends roughly 7 cm above
    the visible ledge, so objects can be visually embedded or roll off its
    sloped HSM fragments.  Its sibling repaired SDF replaces only that hull with
    a 2.5 cm tray plate at the visible ledge height; all other 31 hulls and the
    visual GLTF are untouched.  No repaired asset means no usable surface: the
    required whiteboard target must fail rather than publish unphysical props.
    """
    if furniture_object.sdf_path is None:
        return None
    sdf_path = Path(furniture_object.sdf_path)
    ...
    if (
        ...
        console_logger.error(
            "Rolling-whiteboard marker-tray repair is unavailable for %s",
            furniture_object.object_id,
        )
        return None

    # The tray's broad visual face is nearly horizontal but HSM retains its
    # small triangulation tilt.  Preserve yaw and center X while using a level,
    # 8 cm deep footprint that exactly matches the repaired convex plate.
    rotation = surface.transform.rotation().matrix()
    yaw = float(np.arctan2(rotation[1, 0], rotation[0, 0]))
    translation = np.asarray(surface.transform.translation(), dtype=float).copy()
    translation[1] = 0.02
    translation[2] = 1.067
    console_logger.info(
        "Prepared 32-hull collision repair and level marker tray %s for %s",
        surface.surface_id,
        furniture_object.object_id,
    )
    return SupportSurface(
        surface_id=surface.surface_id,
        bounding_box_min=np.array([-0.59, -0.04, 0.01], dtype=float),
        bounding_box_max=np.array([0.59, 0.04, 0.07], dtype=float),
        transform=RigidTransform(RotationMatrix.MakeZRotation(yaw), translation),
        mesh=surface.mesh,
...
def extract_and_propagate_support_surfaces(
    ...
    *,
    propagate_to_identical: bool = True,
) -> list[SupportSurface]:
    ...
    Args:
        ...
        propagate_to_identical: Whether to mutate other furniture instances that
            share the same geometry.  Per-furniture manipuland workflows disable
            this so each checkpoint has exactly one mutable support-surface owner.
    ...
    if not surfaces:
        recovered_trays = _recover_rolling_whiteboard_marker_trays(
            furniture_object=furniture_object,
            config=config,
        )
        if recovered_trays:
            surfaces = recovered_trays
            source = "HSM rolling-whiteboard marker-tray recovery"
