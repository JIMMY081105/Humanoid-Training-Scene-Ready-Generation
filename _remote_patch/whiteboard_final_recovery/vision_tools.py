...
class ManipulandVisionTools:
    ...
    def __init__(
        ...
        Args:
            ...
            protected_object_ids: Exact pre-workflow object IDs.  Protected
                manipulands are excluded even if a restored support-surface ID is
                reused by the current furniture.
        """
        self.scene = scene
    ...
    def _observe_scene_impl(self) -> list[ToolOutputImage | ToolOutputText]:
        ...
        # Keep the normal top/side views as model context.  ``*_S_*`` files are
        # support-surface debug overlays: they remain in ``images_dir`` for the
        # checkpoint and human review, while the structured scene-state tool
        # supplies the same surface IDs to the agent.  Returning every overlay
        # needlessly triples the first observation payload for a single surface
        # and can delay the following model turn by several minutes.
        image_paths = sorted(images_dir.glob("*.png"))
        model_image_paths = [path for path in image_paths if "_S_" not in path.stem]
        if not model_image_paths:
            # Preserve visual context for rendering modes that only produce
            # annotated images.
            model_image_paths = image_paths
        omitted_debug_images = len(image_paths) - len(model_image_paths)

        # Collect model-context images and return them directly.
        outputs: list[ToolOutputImage | ToolOutputText] = []

        for img_path in model_image_paths:
        ...
        console_logger.info(
            f"Returning {num_images} model-context images via ToolOutputImage "
            f"({omitted_debug_images} support-debug image(s) retained on disk)"
        )
        return outputs
    ...
    def _check_physics_impl(self) -> str:
        ...
        return check_physics_violations(
            ...
            manipuland_scope_ids=current_scope_ids,
        )
