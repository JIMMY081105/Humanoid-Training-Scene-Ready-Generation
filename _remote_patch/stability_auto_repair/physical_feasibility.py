...
def _use_rolling_whiteboard_marker_tray_collision_repair(furniture) -> None:
    """Switch only an isolated simulation copy to the validated tray hull.

    Checkpoint semantics forbid modifying the source furniture object during
    manipuland placement.  The repaired SDF is therefore used only by the
    temporary Drake subset that validates physical stability; the visual scene
    keeps its original asset path and remains checkpoint-compatible.
    """
    if furniture.sdf_path is None:
        return
    if (
        str(furniture.name or "").casefold() != "whiteboard"
        or "rolling" not in str(furniture.description or "").casefold()
    ):
        return
    sdf_path = Path(furniture.sdf_path)
    repaired_sdf_path = sdf_path.with_name("whiteboard_marker_tray_repaired.sdf")
    repaired_hull_path = sdf_path.with_name("whiteboard_marker_tray_collision_6.obj")
    if (
        repaired_sdf_path.is_symlink()
        or repaired_hull_path.is_symlink()
        or not repaired_sdf_path.is_file()
        or not repaired_hull_path.is_file()
    ):
        raise RuntimeError(
            "Rolling-whiteboard marker-tray collision repair is unavailable"
        )
    furniture.sdf_path = repaired_sdf_path
    console_logger.info(
        "Using repaired rolling-whiteboard marker-tray collision in physics subset"
    )


def clone_scene_with_collision_repairs(scene: RoomScene) -> RoomScene:
    """Clone only repaired furniture for a collision-only Drake query.

    Semantic scenes retain the original asset path for reproducible checkpoints
    and visual rendering.  Collision queries, however, must use the same
    validated tray hull as the final stability simulation; otherwise a valid
    marker is lifted away from its physical support merely to clear the stale
    coarse hull.  The shallow scene copy shares immutable room data and clones
    only furniture whose collision SDF is replaced.
    """
    collision_scene = copy(scene)
    collision_scene.objects = dict(scene.objects)
    repaired_count = 0

    for object_id, obj in scene.objects.items():
        if obj.object_type != ObjectType.FURNITURE:
            continue
        collision_furniture = deepcopy(obj)
        original_sdf_path = collision_furniture.sdf_path
        _use_rolling_whiteboard_marker_tray_collision_repair(collision_furniture)
        if collision_furniture.sdf_path != original_sdf_path:
            collision_scene.objects[object_id] = collision_furniture
            repaired_count += 1

    if repaired_count:
        console_logger.info(
            "Using repaired collision geometry for %s furniture object(s) in "
            "collision validation",
            repaired_count,
        )
    return collision_scene
...
def solve_non_penetration_ik(
    ...
    if solver_name == "snopt":
        ...
        options.SetOption(solver.id(), "max_cpu_time", time_limit_s)
    else:
        raise ValueError(f"Invalid solver: {solver_name}")

    try:
        result = _solve_projection_with_timeout(
            solver=solver,
            prog=prog,
            options=options,
...
def _solve_projection_with_timeout(*, solver, prog, options, time_limit_s: float):
    """Solve with a process alarm only where Python permits installing one.

    SNOPT and IPOPT are configured with their own time/iteration limits before this
    helper is called. On POSIX, the main thread also gets a SIGALRM grace-period
    guard because SNOPT can occasionally ignore its internal limit. Python forbids
    installing signal handlers from worker threads, so those calls deliberately rely
    on the configured solver limits instead of failing before the solver starts.
    """
    can_use_process_alarm = (
        hasattr(signal, "SIGALRM")
        and threading.current_thread() is threading.main_thread()
    )
    if not can_use_process_alarm:
        console_logger.debug(
            "Projection is not running on a POSIX main thread; relying on the "
            "solver's configured time and iteration limits"
        )
        return solver.Solve(prog, None, options)
...
def _declared_support_owner_id(
    scene: RoomScene, manipuland_id: UniqueID
) -> UniqueID | None:
    """Return the scene object that owns a manipuland's declared support."""
    manipuland = scene.get_object(manipuland_id)
    if manipuland is None or manipuland.placement_info is None:
        return None

    parent_surface_id = manipuland.placement_info.parent_surface_id
    for obj in scene.objects.values():
        if any(
            surface.surface_id == parent_surface_id for surface in obj.support_surfaces
        ):
            return obj.object_id
    return None


def _is_declared_support_contact(
    scene: RoomScene,
    object_a_id: str,
    object_b_id: str,
    penetration_depth: float,
    maximum_penetration_m: float = 0.02,
) -> bool:
    """Whether a mild pair is a manipuland contacting its declared parent.

    A support surface is a deliberate physical relationship, not free-space
    overlap.  The per-furniture gate validates these contacts before they enter
    the whole-room pass.  Keep the exception narrow: it applies only to the
    recorded parent surface and only within the same 2 cm tolerance used by
    that gate; every other penetration remains actionable.
    """
    if penetration_depth > maximum_penetration_m:
        return False

    for manipuland_id, parent_id in (
        (UniqueID(object_a_id), UniqueID(object_b_id)),
        (UniqueID(object_b_id), UniqueID(object_a_id)),
    ):
        manipuland = scene.get_object(manipuland_id)
        if manipuland is None or manipuland.object_type != ObjectType.MANIPULAND:
            continue
        if _declared_support_owner_id(scene, manipuland_id) == parent_id:
            return True
    return False


def _apply_declared_support_contact_filtering(
    scene: RoomScene,
    plant: MultibodyPlant,
    scene_graph: SceneGraph,
    object_indices: dict[UniqueID, tuple[ModelInstanceIndex, BodyIndex]],
    support_contact_pairs: set[tuple[UniqueID, UniqueID]],
) -> int:
    """Exclude already-validated parent-support contacts from whole-room IK.

    The global projection searches for free-space clearance.  Applying that
    constraint to a notebook resting in its declared desk or a bin inside its
    declared cubby asks Drake to eliminate the very support contact it has
    already validated, making the optimization infeasible.  This filter is
    used only by the kinematic whole-room projection; per-furniture dynamic
    simulation still models the support normally.
    """
    if not support_contact_pairs:
        return 0

    inspector = scene_graph.model_inspector()
    filter_manager = scene_graph.collision_filter_manager()

    def model_geometry_ids(model_idx: ModelInstanceIndex) -> list:
        geometry_ids = []
        for body_idx in plant.GetBodyIndices(model_idx):
            frame_id = plant.GetBodyFrameIdOrThrow(body_idx)
            geometry_ids.extend(inspector.GetGeometries(frame_id, Role.kProximity))
        return geometry_ids

    filtered_pairs = 0
    for object_a_id, object_b_id in support_contact_pairs:
        object_a_indices = object_indices.get(object_a_id)
        object_b_indices = object_indices.get(object_b_id)
        if object_a_indices is None or object_b_indices is None:
            continue
        geometry_a = model_geometry_ids(object_a_indices[0])
        geometry_b = model_geometry_ids(object_b_indices[0])
        if not geometry_a or not geometry_b:
            continue
        filter_manager.Apply(
            CollisionFilterDeclaration().ExcludeBetween(
                GeometrySet(geometry_a), GeometrySet(geometry_b)
            )
        )
        filtered_pairs += 1
    ...
    if filtered_pairs:
        console_logger.info(
            "Excluded %s declared support contact pair(s) from whole-room IK",
            filtered_pairs,
        )
    return filtered_pairs


def _get_collision_projection_scope(
    scene: RoomScene, penetration_threshold: float = 1e-5
) -> tuple[set[UniqueID], set[tuple[UniqueID, UniqueID]]]:
    ...
    Returns:
        A tuple containing the actionable object IDs and the narrow set of
        already-validated declared support-contact pairs.
    """
    ...
    declared_support_pairs: set[tuple[UniqueID, UniqueID]] = set()
    for collision in collisions:
        object_a_id = UniqueID(collision.object_a_id)
        object_b_id = UniqueID(collision.object_b_id)
        if _is_declared_support_contact(
            scene=scene,
            object_a_id=collision.object_a_id,
            object_b_id=collision.object_b_id,
            penetration_depth=collision.penetration_depth,
        ):
            declared_support_pairs.add(tuple(sorted((object_a_id, object_b_id))))
            continue
    ...
    if declared_support_pairs:
        console_logger.info(
            "Classified %s mild declared support contact pair(s) as expected",
            len(declared_support_pairs),
        )
    return colliding_ids, declared_support_pairs


def _get_colliding_object_ids(
    scene: RoomScene, penetration_threshold: float = 1e-5
) -> set[UniqueID]:
    """Return actionable colliding IDs while preserving the public helper API.

    Whole-room projection also needs the narrow declared-support pairs so it can
    avoid constraining validated support contacts.  That extra detail belongs to
    the private projection-scope helper; existing callers and tests retain the
    original set-only return contract.
    """
    colliding_ids, _ = _get_collision_projection_scope(
        scene=scene, penetration_threshold=penetration_threshold
    )
...
def apply_non_penetration_projection(
    ...
    if total_objects > large_scene_optimization_threshold:
        colliding_ids, declared_support_pairs = _get_collision_projection_scope(
    ...
    try:
        ...
        _apply_declared_support_contact_filtering(
            scene=scene,
            plant=plant,
            scene_graph=scene_graph,
            object_indices=object_indices,
            support_contact_pairs=declared_support_pairs,
        )
...
def _apply_floor_penetration_fallback(
    ...
    def translate_object_world_z(obj, dz: float) -> None:
        """Move an object and serialized composite members by one world-Z delta."""
        delta = RigidTransform(p=[0.0, 0.0, dz])
        obj.transform = delta @ obj.transform
        composite_type = obj.metadata.get("composite_type")
        if composite_type in ("stack", "pile"):
            for member in obj.metadata.get("member_assets", []):
                if "transform" in member:
                    member["transform"] = serialize_rigid_transform(
                        delta @ deserialize_rigid_transform(member["transform"])
                    )
            ...
            if container_asset and "transform" in container_asset:
                container_asset["transform"] = serialize_rigid_transform(
                    delta
                    @ deserialize_rigid_transform(container_asset["transform"])
                )
            for fill_asset in obj.metadata.get("fill_assets", []):
                if "transform" in fill_asset:
                    fill_asset["transform"] = serialize_rigid_transform(
                        delta @ deserialize_rigid_transform(fill_asset["transform"])
                    )

    # Lift each penetrating furniture piece, carrying objects placed on its
    # declared support surfaces.  Moving a cabinet without its contents would
    # manufacture a new cabinet-content penetration and invalidate a previously
    # accepted placement.
    lifted_count = 0
    for furn_id, penetration in furniture_penetrations.items():
        if penetration > 0:
            ...
            translate_object_world_z(obj, lift_amount)

            owned_surface_ids = {
                surface.surface_id for surface in obj.support_surfaces
            }
            moved_dependents = 0
            for candidate in scene.objects.values():
                placement_info = candidate.placement_info
                if (
                    candidate.object_id != furn_id
                    and placement_info is not None
                    and placement_info.parent_surface_id in owned_surface_ids
                ):
                    translate_object_world_z(candidate, lift_amount)
                    moved_dependents += 1

            console_logger.info(
                f"Floor fallback: lifted {obj.name} ({obj.object_id}) by "
                f"{lift_amount*1000:.1f}mm (penetration was {penetration*1000:.1f}mm); "
                f"moved {moved_dependents} declared dependent(s)"
            )
            lifted_count += 1
...
def apply_physical_feasibility_postprocessing(
    ...
    """
    # The semantic scene must continue to reference its original visual assets
    # and checkpoint-compatible SDF paths.  Physics, however, must use the
    # validated collision repair for the rolling-whiteboard marker tray.  Use
    # a temporary scene only when a repair was actually applied, then merge its
    # resulting poses/removals back without leaking the repaired SDF path.
    semantic_scene = scene
    collision_scene = clone_scene_with_collision_repairs(scene)
    repaired_object_ids = [
        object_id
        for object_id, semantic_object in semantic_scene.objects.items()
        if collision_scene.objects.get(object_id) is not semantic_object
    ]
    if repaired_object_ids:
        scene = collision_scene
        console_logger.info(
            "Running post-processing with repaired collision geometry for %s",
            repaired_object_ids,
        )

    # A final manipuland pass welds furniture in place.  Correct any real
    # furniture-floor embed before constructing IK so a fixed cabinet cannot
    # make the whole-room constraint system infeasible.  This is a measured
    # geometric correction (penetration depth + 1 mm), not a tolerance waiver.
    if weld_furniture and projection_enabled:
        scene, lifted_count = _apply_floor_penetration_fallback(
            scene=scene, margin_m=0.001
        )
        if lifted_count:
            console_logger.info(
                "Pre-projection floor correction lifted %s welded furniture piece(s)",
                lifted_count,
            )
    ...
    # Stage 2: Simulation (runs regardless of projection result), except for
    # the large welded whole-room pass.  Every manipuland reaches that pass
    # only after its own support-aware projection and dynamic settling gate.
    # Re-simulating hundreds of welded furniture/manipuland bodies treats
    # those certified support contacts as a new free-body system, can eject
    # contained assets through coarse cabinet collision hulls, and has no new
    # stability information to add.  The global projection above still checks
    # every *actionable* cross-object/floor penetration.
    removed_ids: list[UniqueID] = []
    if simulation_enabled:
        is_large_welded_global_pass = (
            weld_furniture
            and len(scene.objects) > large_scene_optimization_threshold
        )
        if is_large_welded_global_pass:
            console_logger.info(
                "Skipping redundant large welded whole-room simulation; "
                "per-furniture dynamic gates and global actionable-collision "
                "projection already passed"
            )
        else:
    ...
    if repaired_object_ids:
        # The temporary scene shares every unmodified object with the semantic
        # scene.  Only repaired furniture was cloned, so copy just its settled
        # pose back and retain the original visual/checkpoint asset reference.
        for object_id in repaired_object_ids:
            processed_object = scene.get_object(object_id)
            semantic_object = semantic_scene.get_object(object_id)
            if processed_object is not None and semantic_object is not None:
                semantic_object.transform = RigidTransform(
                    processed_object.transform.GetAsMatrix4()
                )

        # Removals were made from the temporary scene's object dictionary.
        # Apply the same decision to the semantic scene returned to callers.
        for object_id in removed_ids:
            if semantic_scene.get_object(object_id) is not None:
                semantic_scene.remove_object(object_id)
...
def apply_per_furniture_postprocessing(
    ...
    Args:
        ...
        manipuland_ids: Exact manipulands created by the current furniture
            workflow.  Ownership is never inferred from surface IDs.
    ...
    is_floor_target = bool(
        full_scene.room_geometry
        and full_scene.room_geometry.floor
        and full_scene.room_geometry.floor.object_id == furniture_id
    )

    # Add walls and floor.  A floor target already lives in room_geometry and must
    # not be added as a second model in subset_objects.
    for obj in full_scene.objects.values():
        if obj.object_type == ObjectType.WALL or (
            obj.object_type == ObjectType.FLOOR and not is_floor_target
        ):
            subset_objects[obj.object_id] = obj

    # Add target furniture.  The simulation subset owns its own copy, so an
    # asset-specific collision repair can never mutate the checkpoint-protected
    # semantic scene object.
    ...
    if is_floor_target:
        # Floor items need every furniture obstacle as welded collision context.
        for obj in full_scene.objects.values():
            if obj.object_type == ObjectType.FURNITURE:
                subset_objects[obj.object_id] = obj
    else:
        simulation_furniture = deepcopy(furniture)
        _use_rolling_whiteboard_marker_tray_collision_repair(simulation_furniture)
        subset_objects[simulation_furniture.object_id] = simulation_furniture

    # Add only explicitly current-scope manipulands.  A restored scene can contain
    # legacy surface IDs, so surface-only discovery is not an ownership boundary.
    if len(set(manipuland_ids)) != len(manipuland_ids):
        raise RuntimeError("Per-furniture manipuland scope contains duplicate IDs")
    valid_surface_ids = {surface.surface_id for surface in furniture.support_surfaces}
    for manip_id in manipuland_ids:
        manip = full_scene.get_object(manip_id)
        if manip is None:
            raise RuntimeError(
                f"Current-scope manipuland {manip_id} disappeared before post-processing"
            )
        if manip.object_type != ObjectType.MANIPULAND:
            raise RuntimeError(
                f"Current-scope object {manip_id} is not a manipuland"
            )
        if (
            manip.placement_info is None
            or manip.placement_info.parent_surface_id not in valid_surface_ids
        ):
            raise RuntimeError(
                f"Current-scope manipuland {manip_id} is not placed on "
                f"furniture {furniture_id}"
            )
    ...
    # The critic's accepted render is the durable design evidence.  The subset
    # scene deliberately shares SceneObject instances with the full scene, so
    # take immutable pose snapshots before physics is allowed to mutate them.
    # Contained storage (for example cubbies) is not always represented by
    # watertight collision geometry; a free-body simulation can otherwise eject
    # a valid, accepted item through its shelf.  Such an output must never be
    # merged or silently remove evidence-backed manipulands.
    original_transforms = {
        manip_id: RigidTransform(
            full_scene.get_object(manip_id).transform.GetAsMatrix4()
        )
        for manip_id in manipuland_ids
    }
    ...
    unstable_object_ids: set[UniqueID] = set()
    for manip_id, original_transform in original_transforms.items():
        processed_manip = processed_subset.get_object(manip_id)
        if processed_manip is None:
            unstable_ids.append(f"{manip_id} (removed)")
            unstable_object_ids.add(manip_id)
            continue
        translation_delta_m = float(
            np.linalg.norm(
                processed_manip.transform.translation()
                - original_transform.translation()
            )
        )
        relative_rotation = (
            processed_manip.transform.rotation().matrix()
            @ original_transform.rotation().matrix().T
        )
        ...
        # A settled contact may move a few millimetres under gravity.  Larger
        # travel or a visible rotation means the object was not stably supported
        # and must not be accepted merely because an earlier render looked good.
        if translation_delta_m > 0.02 or rotation_delta_deg > 5.0:
            unstable_object_ids.add(manip_id)
            unstable_ids.append(
                f"{manip_id} (translation={translation_delta_m:.3f}m, "
                f"rotation={rotation_delta_deg:.1f}°)"
            )
    ...
    if unstable_ids:
        ...
        second_unstable_object_ids: set[UniqueID] = set()
        for manip_id in manipuland_ids:
        ...
        if len(settled_transforms) == len(manipuland_ids):
            ...
            for manip_id, settled_transform in settled_transforms.items():
                ...
                if revalidated_manip is None:
                    second_unstable_object_ids.add(manip_id)
            ...
            elif revalidation_success and second_unstable_object_ids:
                # Keep no object merely because it looked correct in a render.
                # Remove only objects that still violate the strict 2 cm / 5 degree
                # limits, then independently simulate every survivor a third time.
                # This avoids rolling back a whole required target because a few
                # optional thin binders, bookends, or loose stacks tip over, while
                # preserving the exact stability threshold for every delivered item.
                for manip_id in second_unstable_object_ids:
                    if revalidated_subset.get_object(manip_id) is not None:
                        revalidated_subset.remove_object(manip_id)
                retained_ids = [
                    manip_id
                    for manip_id in manipuland_ids
                    if manip_id not in second_unstable_object_ids
                    and revalidated_subset.get_object(manip_id) is not None
                ]
                survivor_issues: list[str] = []
                if not retained_ids:
                    survivor_issues.append("no stable manipulands remain after pruning")
                else:
                    survivor_transforms = {
                        manip_id: RigidTransform(
                            revalidated_subset.get_object(manip_id).transform.GetAsMatrix4()
                        )
                        for manip_id in retained_ids
                    }
                    survivor_subset, survivor_success, _ = (
                        apply_physical_feasibility_postprocessing(
                            scene=revalidated_subset,
                            weld_furniture=True,
                            projection_enabled=projection_cfg.enabled,
                            projection_influence_distance=(
                                projection_cfg.influence_distance
                            ),
                            projection_solver_name=projection_cfg.solver_name,
                            projection_iteration_limit=projection_cfg.iteration_limit,
                            projection_time_limit_s=projection_cfg.time_limit_s,
                    ...
                    if not survivor_success:
                        survivor_issues.append(
                            "post-prune physical pass reported issues"
                        )
                    for manip_id, stable_transform in survivor_transforms.items():
                        survivor = survivor_subset.get_object(manip_id)
                        if survivor is None:
                            survivor_issues.append(
                                f"{manip_id} (removed during post-prune validation)"
                            )
                            continue
                        translation_delta_m = float(
                            np.linalg.norm(
                                survivor.transform.translation()
                                - stable_transform.translation()
                            )
                        )
                        relative_rotation = (
                            survivor.transform.rotation().matrix()
                            @ stable_transform.rotation().matrix().T
                        )
                        rotation_delta_deg = float(
                            np.degrees(
                                np.arccos(
                        ...
                        if translation_delta_m > 0.02 or rotation_delta_deg > 5.0:
                            survivor_issues.append(
                                f"{manip_id} (post-prune translation="
                                f"{translation_delta_m:.3f}m, post-prune rotation="
                                f"{rotation_delta_deg:.1f} degrees)"
                            )

                    if not survivor_issues:
                        processed_subset = survivor_subset
                        removed_names = ", ".join(
                            sorted(str(item) for item in second_unstable_object_ids)
                        )
                        unstable_ids = []
                        revalidation_issues = []
                        console_logger.warning(
                            "Removed repeatedly unstable optional manipulands from %s "
                            "after strict survivor revalidation: %s",
                            furniture_id,
                            removed_names,
                        )

                if survivor_issues:
                    revalidation_issues.extend(survivor_issues)
