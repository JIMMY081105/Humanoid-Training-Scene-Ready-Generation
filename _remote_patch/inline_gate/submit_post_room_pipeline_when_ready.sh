...
readonly -a RECOVERY_JOBS=(
  'classroom_04|169026|paracloud_classroom04_manipuland_resume.sbatch'
  'storage_room|169049|paracloud_storage_manipuland_resume.sbatch'
  'classroom_05|169061|paracloud_classroom05_manipuland_resume.sbatch'
  'library|169072|paracloud_manipuland_resume.sbatch'
  'boys_toilet|169073|paracloud_manipuland_resume.sbatch'
  'classroom_06|169083|paracloud_manipuland_resume.sbatch'
  'classroom_02|169104|paracloud_manipuland_resume.sbatch'
  'classroom_03|169105|paracloud_manipuland_resume.sbatch'
)
...
# Verify every queued recovery will execute the release gate before exit.
dependencies=()
for entry in "${RECOVERY_JOBS[@]}"; do
  IFS='|' read -r room_id job_id script_name <<< "$entry"
  script="$FACTORY/$script_name"
  test -s "$script"
  grep -q 'RECOVERY_AND_RELEASE_GATE_PASS' "$script"
  grep -q 'paracloud_room_release_gate.sbatch' "$script"
