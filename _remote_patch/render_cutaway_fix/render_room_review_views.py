"""Render five hash-bound, provably cutaway review angles per room."""
...
VIEW_NAMES = ("top", "oblique_a", "oblique_b", "detail_a", "detail_b")
...
def classify_room_envelope(
    ...
    # Contract-detail meshes may extend slightly below the authored floor and
    # therefore expand the all-mesh Z bounds.  That must not make the genuine
    # full-height, thin, authored boundary walls look disconnected from the
    # floor.  Recover only strict wall-shaped boundary meshes, and only when a
    # separately classified floor proves the local floor height.  This keeps
    # the cutaway fail-closed for whiteboards, displays, dividers, and other
    # content that is merely tall or thin.
    if classified["floor"]:
        floor_top = max(record["maximum"][2] for record in classified["floor"])
        thin_limit = max(0.16, 0.02 * min(room_bounds["width"], room_bounds["depth"]))
        boundary_tolerance = max(0.20, 1.5 * thin_limit)
        recovered: list[dict[str, Any]] = []
        retained: list[dict[str, Any]] = []
        for record in classified["content"]:
            minimum = record["minimum"]
            maximum = record["maximum"]
            ...
            thin_x_wall = (
                dx <= thin_limit
                and dy >= 0.75 * room_bounds["depth"]
                and min(
                    abs(center_x - room_bounds["minimum"][0]),
                    abs(center_x - room_bounds["maximum"][0]),
                ) <= boundary_tolerance
            )
            thin_y_wall = (
                dy <= thin_limit
                and dx >= 0.75 * room_bounds["width"]
                and min(
                    abs(center_y - room_bounds["minimum"][1]),
                    abs(center_y - room_bounds["maximum"][1]),
                ) <= boundary_tolerance
            )
            strict_boundary_wall = (
                minimum[2] <= floor_top + 0.16
                and dz >= 2.2
                and (thin_x_wall or thin_y_wall)
            )
            if strict_boundary_wall:
                recovered.append(
                    {
                        **record,
                        "role": "wall",
                        "classification_reason": "authored_boundary_wall_recovered_from_floor_height",
                    }
                )
            else:
                retained.append(record)
        classified["wall"].extend(recovered)
        classified["content"] = retained
    ...
    *,
    ...
    try:
        ...
        if room_id == "classroom_01":
            # Room 1 deliberately retains its two acceptance-detail views: the
            # cubby supplies and the complete north teaching wall.
            cubby_target = center + Vector(
                (-width * 0.40, -depth * 0.25, -room_bounds["height"] * 0.18)
            )
            teaching_target = center + Vector(
                (0.0, depth * 0.34, -room_bounds["height"] * 0.14)
            )
            views = (
                (
                    "top",
                ...
                (
                    ...
                    teaching_target,
                    max(width, depth) * 0.86,
                ),
            )
        else:
            # Every other room requires two complete-room, complementary
            # cutaways.  Applying Room 1's tiny cubby crop globally produced a
            # blank restroom view and hid corridor thresholds.
            target = center
            ...
            if room_id == "boys_toilet":
                # Preserve the north sink wall in one view so both real mirrors
                # and the drying station remain visible; the opposite cutaway
                # continues to show the toilet/stall arrangement.
                oblique_a_offset = Vector(
                    (camera_distance, -camera_distance, camera_distance * 1.25)
                )
                ...
                # Complementary north/south cutaways prove both the entrance
                # welcome zone and the warm north-wall lobby fixtures.
                oblique_a_offset = Vector(
                    (-camera_distance, -camera_distance, camera_distance * 1.25)
                )
                oblique_b_offset = Vector(
            ...
            diagonal_scale = (
                math.sqrt(
                    width * width + depth * depth + room_bounds["height"] ** 2
                )
                * 1.15
            )
            views = (
            ...
            if room_id == "boys_toilet":
                sink_target = center + Vector(
                    (0.0, depth * 0.27, -room_bounds["height"] * 0.12)
                )
                stall_target = center + Vector(
                    (-width * 0.22, 0.0, -room_bounds["height"] * 0.12)
                )
                views = (
                    views[0],
                    (
                    ...
                    (
                        ...
                        + Vector(
                            (
                                camera_distance * 0.90,
                                -camera_distance * 0.85,
                                camera_distance * 0.80,
                            )
                        ),
                        stall_target,
                        max(width, depth) * 1.08,
                    ),
                )
            elif room_id in {"classroom_02", "classroom_03", "classroom_04"}:
                # A full top view proves the 12 desk-chair pairs and circulation;
                # the two complementary views now prove the teacher supply
                # station and the daylight/teaching wall at a resolution where
                # required small objects are actually reviewable.
                supply_offsets = {
                    "classroom_02": (0.30, 0.26),
                    "classroom_03": (-0.17, 0.29),
                    "classroom_04": (-0.29, 0.36),
                }
                fx, fy = supply_offsets[room_id]
                supply_target = center + Vector(
                    (width * fx, depth * fy, -room_bounds["height"] * 0.17)
                )
                teaching_target = center + Vector(
                    (0.0, depth * 0.30, -room_bounds["height"] * 0.13)
                )
                views = (
                    views[0],
                    (
                        ...
                        + Vector(
                            (
                                ...
                                -camera_distance * 0.72,
                                camera_distance * 0.50,
                            )
                        ),
                        supply_target,
                        max(width, depth) * 0.28,
                    ),
                    (
                        "oblique_b",
                        teaching_target
                        + Vector(
                            (
                                -camera_distance * 0.82,
                                -camera_distance * 0.62,
                                camera_distance * 0.62,
                            )
                        ),
                        teaching_target,
                        max(width, depth) * 0.82,
                    ),
                )
            elif room_id == "main_corridor":
                entrance_target = center + Vector(
                    (0.0, -depth * 0.38, -room_bounds["height"] * 0.10)
                )
                lobby_target = center + Vector(
                    (0.0, depth * 0.12, -room_bounds["height"] * 0.08)
                )
                views = (
                    views[0],
                    (
                    ...
                    (
                        ...
                        lobby_target,
                        max(width, depth) * 0.48,
                    ),
                )
            elif room_id == "girls_toilet":
                sink_target = center + Vector(
                    (width * 0.30, -depth * 0.15, -room_bounds["height"] * 0.12)
                )
                stall_target = center + Vector(
                    (-width * 0.10, depth * 0.27, -room_bounds["height"] * 0.12)
                )
                views = (
                    views[0],
                    (
        ...
        # The strict school contract contains many small-but-required objects.
        # Two additional, still hash-bound cutaways make the storage/doorway
        # and daylight/lighting evidence reviewable without relaxing any gate.
        detail_targets = {
            "classroom_01": ((0.42, 0.00), (0.00, 0.39)),
            "classroom_02": ((0.31, -0.39), (-0.43, 0.00)),
            "classroom_03": ((-0.31, -0.39), (0.43, 0.00)),
            "classroom_04": ((0.00, -0.32), (0.22, -0.40)),
            "boys_toilet": ((-0.28, 0.16), (0.43, -0.35)),
            "girls_toilet": ((-0.24, 0.20), (0.43, -0.35)),
            "main_corridor": ((0.40, -0.36), (0.00, 0.02)),
        }
        (a_fx, a_fy), (b_fx, b_fy) = detail_targets.get(
            room_id, ((-0.30, -0.30), (0.30, 0.30))
        )
        detail_a_target = center + Vector(
            (width * a_fx, depth * a_fy, -room_bounds["height"] * 0.10)
        )
        detail_b_target = center + Vector(
            (
                width * b_fx,
                depth * b_fy,
                room_bounds["height"] * (0.12 if room_id == "main_corridor" else -0.08),
            )
        )
        # West-side doors need the opposite horizontal viewpoint so their
        # authored opening remains on the far (visible) wall.
        b_x_sign = 1.0 if room_id == "classroom_02" else -1.0
        detail_scale = max(width, depth) * (0.36 if room_id != "main_corridor" else 0.32)
        views = views + (
            (
                "detail_a",
                detail_a_target
        ...
        if [view["view_name"] for view in evidence["views"]] != list(VIEW_NAMES):
            raise CutawayError("Renderer did not produce the exact five-view contract")
