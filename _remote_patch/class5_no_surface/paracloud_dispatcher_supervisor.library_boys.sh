...
while true; do
  ...
  ensure_dispatcher \
    submit_classroom02_recovery_when_slot.sh \
    "$FACTORY/classroom02_gate_replacement_submission.txt" \
    && classroom02_done=1 || true
  ensure_dispatcher \
    submit_classroom03_recovery_when_slot.sh \
    "$FACTORY/classroom03_gate_replacement_submission.txt" \
    && classroom03_done=1 || true
  ensure_dispatcher \
  ...
  ensure_dispatcher \
    submit_library_boys_recovery_when_slot.sh \
    "$FACTORY/library_boys_recovery_submissions_complete.txt" \
    && library_boys_done=1 || true
  ensure_dispatcher \
