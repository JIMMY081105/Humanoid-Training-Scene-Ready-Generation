...
def test_uniform_scaler_corrects_canonical_horizontal_yaw(
    tmp_path: Path,
) -> None:
    # This is a thin, flat asset whose VLM-derived canonical front has put its
    # long horizontal axis on GLTF Z.  The requested width/depth need that axis
    # on X; the scaler may yaw around GLTF Y but must not change the up axis.
    source = _box(tmp_path / "yawed_scissors.glb", [0.132, 0.009, 0.202])
    output = tmp_path / "yawed_scissors_scaled.glb"

    _, _, receipt = scale_mesh_uniformly_to_scene_dimension_contract(
        source,
        requested_scene_dimensions=[0.18, 0.07, 0.015],
        output_path=output,
    )

    assert receipt["yaw_rotation_degrees"] == 90
    np.testing.assert_allclose(
        trimesh.load(output, force="mesh").extents,
        receipt["planned_final_gltf_dimensions_m"],
        rtol=1e-5,
        atol=1e-6,
    )
    measured_scene = gltf_to_scene_dimensions(
        trimesh.load(output, force="mesh").extents
    )
    assert measured_scene[2] < 0.015
    assert measured_scene[0] >= 0.10
    assert measured_scene[1] == pytest.approx(0.07, abs=1e-6)


def test_uniform_scaler_recovers_thin_vertical_axis_after_ambiguous_canonicalization(
    tmp_path: Path,
) -> None:
    # This matches the accepted closed-notebook candidate from Room 1.  Its
    # physical thickness landed on GLTF Z after an ambiguous canonicalization;
    # direct/yaw-only fits must fail, while a rigid axis recovery puts the thin
    # requested scene height back on GLTF Y without distorting proportions.
    source = _box(
        tmp_path / "ambiguous_notebook.glb",
        [0.1732969284, 0.2222501315, 0.0252638841],
    )
    output = tmp_path / "ambiguous_notebook_scaled.glb"

    _, _, receipt = scale_mesh_uniformly_to_scene_dimension_contract(
        source,
        requested_scene_dimensions=[0.19, 0.25, 0.015],
        output_path=output,
    )

    assert receipt["orientation_recovery"] == "axis_recovery"
    assert receipt["yaw_rotation_degrees"] == 0
    measured_scene = gltf_to_scene_dimensions(
        trimesh.load(output, force="mesh").extents
    )
    assert measured_scene[2] == pytest.approx(0.015, abs=1e-6)
    assert measured_scene[0] >= 0.10
    assert measured_scene[1] >= 0.12
