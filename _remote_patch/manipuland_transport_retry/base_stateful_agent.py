...
from scenesmith.agent_utils.room import AgentType, UniqueID
...
_TRANSIENT_MODEL_TRANSPORT_ERROR_MARKERS = (
    "connection error",
    "connectionerror",
    "connecterror",
    "server disconnected",
...
_CRITIC_TRANSPORT_RETRY_ATTEMPTS = 4


class _TraceSafeModelSettings(ModelSettings):
    """Keep provider-only request objects out of OpenAI Agents trace encoding.

    ``ModelSettings.extra_args`` is forwarded to the provider and can therefore
    contain valid non-JSON request objects such as ``openai.Timeout``. OpenAI
    Agents 0.17.4 intends to omit those extras from traces, but serializes the
    entire dataclass before filtering fields, which raises before the request is
    sent. Preserve the real request settings while tracing a copy whose
    provider-only fields are absent.
    """

    def to_traceable_dict(self) -> dict[str, Any]:
        traceable_settings = replace(
            self,
            extra_query=None,
            extra_body=None,
            extra_headers=None,
        ...
        return ModelSettings.to_traceable_dict(traceable_settings)
...
class BaseStatefulAgent(ABC):
    ...
    def _get_model_settings(
        ...
        return _TraceSafeModelSettings(**kwargs) if kwargs else None
    ...
    def _get_manipuland_physics_scope_ids(self) -> set[UniqueID] | None:
        """Return an explicit manipuland ownership scope when applicable."""
        return None

    @staticmethod
    def _is_transient_model_transport_error(error: BaseException) -> bool:
        """Return whether a failed model call is safe to retry unchanged.

        Critic calls only observe the current scene and produce a verdict; they
        never mutate it.  Retrying a verified transport failure is therefore
        safe, unlike retrying a designer or planner turn which could duplicate
        tool side effects.
        """
        seen: set[int] = set()
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                marker in message
                for marker in _TRANSIENT_MODEL_TRANSPORT_ERROR_MARKERS
            ):
                return True
    ...
    async def _run_critic_with_transport_retry(self, critique_instruction: str) -> RunResult:
        """Run the read-only critic with bounded recovery for network failures."""
        for attempt in range(1, _CRITIC_TRANSPORT_RETRY_ATTEMPTS + 1):
            try:
                ...
                if (
                    attempt >= _CRITIC_TRANSPORT_RETRY_ATTEMPTS
                    or not self._is_transient_model_transport_error(error)
                ):
                    raise
                delay_s = float(2 ** (attempt - 1))
                console_logger.warning(
                    "Critic transport failure on attempt %d/%d; retrying in %.1fs: %s",
                    attempt,
                    _CRITIC_TRANSPORT_RETRY_ATTEMPTS,
                    delay_s,
                    error,
                )
                await asyncio.sleep(delay_s)

        raise AssertionError("critic retry loop exhausted without returning or raising")
    ...
    async def _request_critique_impl(self, update_checkpoint: bool = True) -> str:
        ...
        physics_context = check_physics_violations(
            ...
            manipuland_scope_ids=self._get_manipuland_physics_scope_ids(),
        )
        ...
        result = await self._run_critic_with_transport_retry(critique_instruction)
