...
def apply_per_furniture_postprocessing(
    ...
    for manip_id, original_transform in original_transforms.items():
        ...
        if processed_manip is None:
            ...
            continue
        if not np.allclose(
            processed_manip.transform.GetAsMatrix4(),
            original_transform.GetAsMatrix4(),
            rtol=0.0,
            atol=1e-6,
        ):
            unstable_ids.append(f"{manip_id} (pose changed)")
    ...
    if unstable_ids:
        ...
        console_logger.warning(
            "Discarding unstable per-furniture post-processing for %s; "
            "accepted evidence-backed poses are unchanged: %s",
            furniture_id,
            ", ".join(unstable_ids),
        )
        return full_scene
