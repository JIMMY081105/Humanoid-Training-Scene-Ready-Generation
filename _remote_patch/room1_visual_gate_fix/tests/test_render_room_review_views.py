...
def test_oblique_cutaway_hides_content_attached_to_removed_wall() -> None:
    room_objects = _complete_room_objects()
    # This display is thin, elevated, and attached to the east wall.  It must
    # be classified as content (not an envelope wall) and hidden with that wall
    # in the southeast dollhouse view, or it would look unsupported.
    mounted_display = FakeObject(
        "mounted_display", (9.86, 2.0, 1.0), (10.04, 5.0, 2.2)
    )
    FAKE_BPY.context.scene.objects.extend([*room_objects, mounted_display])
    records, bounds = _room_mesh_records(x=0, y=0, width=10, depth=8)
    classification = classify_room_envelope(records, bounds)

    assert any(
        record["name"] == "mounted_display" for record in classification["content"]
    )
    evidence = establish_cutaway(
        classification=classification,
        room_bounds=bounds,
    ...
    assert [record["name"] for record in evidence["hidden_camera_side_attachments"]] == [
        "mounted_display"
    ]
    assert mounted_display.hide_render is True
    assert "mounted_display" not in evidence["visible_content_object_names"]


def test_cutaway_hides_content_attached_to_removed_overhead() -> None:
    room_objects = _complete_room_objects()
    ceiling_troffer = FakeObject(
        "ceiling_troffer", (4.0, 3.0, 2.9), (5.0, 4.0, 3.2)
    )
    FAKE_BPY.context.scene.objects.extend([*room_objects, ceiling_troffer])
    records, bounds = _room_mesh_records(x=0, y=0, width=10, depth=8)
    classification = classify_room_envelope(records, bounds)

    assert any(
        record["name"] == "ceiling_troffer" for record in classification["content"]
    )
    evidence = establish_cutaway(
        classification=classification,
        room_bounds=bounds,
    ...
    assert [record["name"] for record in evidence["hidden_overhead_attachments"]] == [
        "ceiling_troffer"
    ]
    assert ceiling_troffer.hide_render is True
    assert "ceiling_troffer" not in evidence["visible_content_object_names"]
