...
def render_room(
    ...
    *,
    ...
    try:
        ...
        cubby_daylight_scale = max(width, depth) * 0.18
        ...
        # Room 1's strict visual contract additionally requires evidence of
        # the complete north teaching wall: whiteboard, clock, educational
        # display, and exterior window.  This remains a real cutaway view,
        # but uses a wide enough framing to show that wall end-to-end.
        if room_id == "classroom_01":
            teacher_teaching_target = center + Vector(
                (0.0, depth * 0.34, -room_bounds["height"] * 0.14)
            )
            teacher_teaching_scale = max(width, depth) * 0.86
