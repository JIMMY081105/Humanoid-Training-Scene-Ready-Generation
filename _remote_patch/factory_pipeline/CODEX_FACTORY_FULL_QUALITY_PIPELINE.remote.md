...
> Implementation status (2026-07-13): the generic quality machinery (SAM3D preflight, Artiverse/ArtVIP
> routing, materials contract, ObjectThor retrieval, proxy verification, room worker, assembly, and
> Drake/MuJoCo validation) is reused unchanged. Factory-specific acceptance siblings are required
> because the existing acceptance scripts bind the school room set and school gate labels. The factory-specific `[BUILD]`
> sources and runner modes named below now exist in the isolated factory checkout. `[BUILD]` remains a
> normative gate marker: generation is still forbidden until the current source hashes are covered by
> `pipeline_code_contract.json` and every no-GPU code/input/model/asset/router preflight passes.
...
> Immutable factory input binding (2026-07-13): `effective_prompt_chars=21330`,
> `factory_effective_prompt_sha256=35387fa3fffb0af92b6c9e07e39f2b59a5443d497190f721090f82bd81734322`,
> `original_prompt_sha256=3bb58cf8e07012cc0c5381eb51730da03b25fb6659a08338238d9fd4baa5093b`,
> `appendix_sha256=ffc327f082431d407c15b25b039d7d2676786a03663f186777558e9ba8e1db19`, and
> `factory_contract_sha256=f33e56eef4375d1f68684bd10c02d7d2f82ee72420b8a12570e8fcbe135a94cd`.
> No user reference image was supplied; the complete source prompt plus immutable appendix is the sole
> reference authority and `reference_image_sha256` remains JSON null.
...
staging, schema-v2 validation, `factory_final_acceptance_bundle.py` rewalk). Restore the isolated `.mujoco_venv`
...
Inherited from the school contract with factory inputs/counts, implemented by a factory sibling because
the existing bundler is school-hash/room-label specific. Run
`scripts/factory_final_acceptance_bundle.py ... --input-dir inputs/full_quality_factory_reference_<YYYYMMDD>` to
...
and SQZ/ParaCloud completion receipt with `scripts/factory_two_gpu_drake_acceptance_contract.py`, and run
`remote_jobs/TEMPLATE_factory_2gpu_acceptance.sbatch` for the terminal receipt with a real synchronized
