#!/usr/bin/env python3
"""Build the exact food-factory floor plan with native SceneSmith geometry.

This is a deterministic replacement for stochastic LLM room placement for the
single, immutable ``factory_reference_20260713`` production profile.  It does
...
ROOMS: tuple[RoomSeed, ...] = (
    RoomSeed("ingredient_receiving", "receiving", 0.0, 22.0, 10.0, 10.0, ("north", "west")),
    RoomSeed("dry_storage", "storage", 10.0, 24.0, 18.0, 8.0, ("north",)),
    RoomSeed("cold_storage", "cold_storage", 28.0, 24.0, 16.0, 8.0, ("north", "east")),
    RoomSeed("washing_preparation", "food_preparation", 0.0, 13.0, 10.0, 9.0, ("west",)),
    RoomSeed("qc_laboratory", "laboratory", 0.0, 7.0, 10.0, 6.0, ("west",)),
    RoomSeed("office_administration", "office", 0.0, 0.0, 10.0, 7.0, ("west", "south")),
    RoomSeed("processing_hall", "food_processing", 10.0, 14.0, 28.0, 10.0),
    RoomSeed("packaging_hall", "food_packaging", 10.0, 6.0, 28.0, 8.0),
    RoomSeed("finished_goods_storage", "finished_goods_storage", 10.0, 0.0, 34.0, 6.0, ("south", "east")),
    RoomSeed("maintenance", "maintenance", 38.0, 19.0, 6.0, 5.0, ("east",)),
    RoomSeed("changing_room", "changing_room", 38.0, 15.0, 6.0, 4.0, ("east",)),
    RoomSeed("break_room", "break_room", 38.0, 10.0, 6.0, 5.0, ("east",)),
    RoomSeed("boys_toilet", "restroom", 38.0, 8.0, 6.0, 2.0, ("east",)),
    RoomSeed("girls_toilet", "restroom", 38.0, 6.0, 6.0, 2.0, ("east",)),
)

DOORS: tuple[DoorSeed, ...] = (
    DoorSeed("door_receiving_dry", "ingredient_receiving", "dry_storage", width=1.4),
    DoorSeed("door_receiving_washing", "ingredient_receiving", "washing_preparation", center_world=(5.0, 22.0), width=3.0),
    DoorSeed("door_dry_cold", "dry_storage", "cold_storage", width=1.4),
    DoorSeed("door_dry_processing", "dry_storage", "processing_hall", center_world=(19.0, 24.0), width=3.0),
    DoorSeed("door_cold_processing", "cold_storage", "processing_hall", center_world=(33.0, 24.0), width=3.0),
    DoorSeed("door_washing_processing", "washing_preparation", "processing_hall", center_world=(10.0, 20.5), width=3.0),
    DoorSeed("door_qc_washing", "qc_laboratory", "washing_preparation", center_world=(9.0, 13.0)),
    DoorSeed("door_qc_packaging", "qc_laboratory", "packaging_hall"),
    DoorSeed("door_office_qc", "office_administration", "qc_laboratory", center_world=(9.0, 7.0)),
    DoorSeed("door_office_finished", "office_administration", "finished_goods_storage"),
    DoorSeed("door_processing_packaging", "processing_hall", "packaging_hall", center_world=(36.0, 14.0), width=3.0),
    DoorSeed("door_entrance_packaging", "packaging_hall", "finished_goods_storage", center_world=(20.0, 6.0), width=1.8),
    DoorSeed("door_packaging_finished_goods", "packaging_hall", "finished_goods_storage", center_world=(32.0, 6.0), width=3.0),
    DoorSeed("door_processing_maintenance", "processing_hall", "maintenance"),
    DoorSeed("door_processing_changing", "processing_hall", "changing_room"),
    DoorSeed("door_packaging_break", "packaging_hall", "break_room"),
    DoorSeed("door_packaging_boys", "packaging_hall", "boys_toilet", center_world=(38.0, 8.6)),
    DoorSeed("door_packaging_girls", "packaging_hall", "girls_toilet", center_world=(38.0, 6.6)),
    DoorSeed("door_maintenance_cold", "maintenance", "cold_storage"),
)

WINDOW_WALLS: dict[str, str] = {
    "ingredient_receiving": "west",
    "dry_storage": "north",
    "cold_storage": "north",
    "washing_preparation": "west",
    "qc_laboratory": "west",
    "office_administration": "south",
    "finished_goods_storage": "south",
    "maintenance": "east",
    "changing_room": "east",
    "break_room": "east",
    "boys_toilet": "east",
...
ROOM_CONTEXT: dict[str, str] = {
    room.room_id: f"Immutable full-quality food-factory zone: {room.room_id}."
    for room in ROOMS
}
...
def _make_door(door_class: Any, /, **kwargs: Any) -> Any:
    """Construct across pinned Door ABIs while preserving leaf-count evidence."""

    leaf_count = int(kwargs.pop("leaf_count", 1))
    try:
        door = door_class(**kwargs, leaf_count=leaf_count)
    except TypeError as exc:
        if "leaf_count" not in str(exc):
            raise
        door = door_class(**kwargs)
        setattr(door, "leaf_count", leaf_count)
    return door
        ...
        for gltf_path in sorted(root.rglob("*.gltf")):
            ...
            for section in ("buffers", "images"):
                ...
                for index, entry in enumerate(entries):
                    ...
                    elif _inside(repo, target):
                        target_scope = "repo"
                        target_path = target.relative_to(repo).as_posix()
                    else:
                        raise SeedError(
                            f"GLTF {gltf_path} {section}[{index}] URI escapes both "
                            f"the generated scene and canonical repo: {uri}"
                        )
                    records.append(
                        {
...
def _structural_layout_projection(document: dict[str, Any]) -> dict[str, Any]:
    ...
    ``factory_room_contract.py bind-layout`` deliberately rewrites the prompt in
...
def _factory_common_zone_metadata() -> list[dict[str, Any]]:
    """Explicit non-room structural/exterior zones bound into the layout."""
    return [
        {"id":"loading_dock","bounds_xy":[44.0,0.0,52.0,8.0],"outside_shell":True,"backing_door_id":"loading_dock_door"},
        {"id":"entrance_transition","bounds_xy":[18.0,0.0,30.0,6.0],"carved_from":["finished_goods_storage"],"backing_door_id":"main_entrance"},
        {"id":"internal_circulation","bounds_xy_parts":[[30.0,0.0,34.0,14.0],[34.0,0.0,44.0,6.0],[34.0,10.0,38.0,24.0],[0.0,20.0,34.0,24.0],[8.0,0.0,10.0,20.0]],"carved_from":["finished_goods_storage","packaging_hall","processing_hall","washing_preparation","ingredient_receiving","office_administration","qc_laboratory"]},
        {"id":"toilet_foyer","bounds_xy":[34.0,6.0,38.0,10.0],"carved_from":["packaging_hall"],"backing_door_ids":["door_packaging_boys","door_packaging_girls"]},
        {"id":"exterior_truck_road","bounds_xy":[52.0,-8.0,60.0,12.0],"outside_shell":True},
        {"id":"landscaping","bounds_xy":[-6.0,-6.0,44.0,0.0],"outside_shell":True},
    ]
...
def _build_native_layout(stage_scene: Path, prompt: str, cfg: Any) -> tuple[Any, Any]:
    ...
    door_window_config = DoorWindowConfig(
        ...
        door_width_max=max(width_range[1], 3.0),
    ...
    entrance = _make_door(
        Door,
        id="main_entrance",
        boundary_label=_boundary_for_exterior(layout, "finished_goods_storage", "south"),
        position_segment="center",
        position_exact=9.0,
        door_type="exterior",
        room_a="finished_goods_storage",
        room_b=None,
        width=2.0,
        height=2.2,
        leaf_count=2,
    ...
    loading_door = _make_door(
        Door,
        id="loading_dock_door",
        boundary_label=_boundary_for_exterior(layout, "finished_goods_storage", "east"),
        position_segment="center",
        position_exact=1.5,
        door_type="exterior",
        room_a="finished_goods_storage",
    ...
    loading_error = tools._apply_door_to_wall(loading_door)
    if loading_error:
        raise SeedError(f"SceneSmith rejected the loading-dock door: {loading_error}")
    layout.doors.append(loading_door)
    ...
    for room_id, direction in WINDOW_WALLS.items():
        ...
        wall_length = math.dist(wall.start_point, wall.end_point)
        window_width = min(
            1.5,
            wall_length - 2.0 * door_window_config.window_segment_margin,
        )
        if window_width < 1.0:
            raise SeedError(f"Exterior wall for {room_id} cannot hold a >=1.0 m window")
        window = Window(
            id=f"window_{room_id}",
            boundary_label=_boundary_for_exterior(layout, room_id, direction),
            position_along_wall=(wall_length - window_width) / 2.0,
            room_id=room_id,
    ...
    common_zones = [
        {
            ...
            "width": 4.0,
            "depth": 6.0,
            "reason": "A pedestrian-only entrance transition separates the south entrance from forklift traffic.",
            "carved_from": ["finished_goods_storage"],
            "connections": [
                {"id": "entrance_to_packaging", "to": "packaging_hall", "backing_door_id": "door_entrance_packaging"},
                {"id": "entrance_to_internal", "to": "internal_circulation", "width": 2.0, "position": [18.0, 3.0], "orientation": "vertical"},
            ],
        },
        {
            "id": "internal_circulation",
            "position": [36.0, 10.0],
            "width": 2.0,
            "depth": 14.0,
            "reason": "A protected east-side worker corridor separates staff circulation from production equipment.",
            "carved_from": ["packaging_hall", "processing_hall"],
            "connections": [
                {"id": "internal_to_maintenance", "to": "maintenance", "backing_door_id": "door_processing_maintenance"},
                {"id": "internal_to_changing", "to": "changing_room", "backing_door_id": "door_processing_changing"},
                {"id": "internal_to_break", "to": "break_room", "backing_door_id": "door_packaging_break"},
                {"id": "internal_to_toilet_foyer", "to": "toilet_foyer", "width": 2.0, "position": [37.0, 10.0], "orientation": "horizontal"},
                {"id": "internal_to_entrance", "to": "entrance_transition", "width": 2.0, "position": [36.0, 12.0], "orientation": "vertical"},
            ],
        },
        {
            "id": "toilet_foyer",
            ...
            "depth": 4.0,
            "reason": (
                "A separate staff foyer gives both toilets independent access without "
                "crossing either restroom or a production machine zone."
            ),
            "carved_from": ["packaging_hall"],
            "connections": [
                {
                    "id": "foyer_boys",
                    "to": "boys_toilet",
                    "backing_door_id": "door_packaging_boys",
                },
                {
                    "id": "foyer_girls",
                    "to": "girls_toilet",
                    "backing_door_id": "door_packaging_girls",
                },
                {"id": "foyer_internal", "to": "internal_circulation", "width": 2.0, "position": [37.0, 10.0], "orientation": "horizontal"},
            ],
        }
    ]
    common_result = FloorPlanTools._set_navigation_common_zones_impl(
        tools,
        json.dumps(common_zones, sort_keys=True)
    )
    if not common_result.success:
        raise SeedError(f"SceneSmith rejected factory toilet foyer: {common_result.message}")

    native_validation = FloorPlanTools._validate_impl(tools)
...
def _validate_document(
    document: dict[str, Any], contract_path: Path | None = None
) -> dict[str, Any]:
    from validate_factory_floor_layout import validate

    result = validate(document, contract_path)
    if result.get("status") != "pass":
        raise SeedError(
            "Reference-factory validator rejected deterministic native layout: "
...
def _verify_existing(
    *,
    ...
    receipt_path = scene_dir / "quality_gates" / "reference_factory_layout_seed.json"
    _regular_file(layout_path, label="existing house layout")
    _regular_file(receipt_path, label="reference-factory seed receipt")
    ...
    if receipt.get("factory_contract") != {
        "path": str(contract_path.resolve()),
        "sha256": _sha256_file(contract_path),
    }:
        raise SeedError("Existing seed receipt does not bind the current factory contract")
    validation = _validate_document(layout_document, contract_path)
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    contract_path = args.factory_contract.expanduser().resolve(strict=True)
    # The shared ParaCloud virtualenv is an editable install whose default
    # source checkout is not this isolated factory tree.  Put the attested
    # checkout first before any lazy SceneSmith import.
    repo_import_path = str(repo)
    if not sys.path or sys.path[0] != repo_import_path:
        sys.path.insert(0, repo_import_path)
    if not _inside(output_root, scene_dir):
        raise SeedError("Scene directory must stay inside the authorized factory output root")
    if not (_inside(repo, config_path) or _inside(output_root, config_path)):
        raise SeedError("Resolved config must stay inside the checkout or factory output root")
    if not _inside(repo, prompt_path):
        raise SeedError("Prompt must stay inside the canonical factory checkout")
    if not _inside(repo, contract_path):
        raise SeedError("Factory contract path must be inside the canonical checkout")
    for path, label in ((config_path, "resolved config"), (prompt_path, "prompt"), (contract_path, "factory contract")):
    ...
    try:
        ...
        layout_document["factory_contract_binding"] = {
            "id": "food_factory_full_quality_20260713",
            "path": "scripts/factory_contract.json",
            "sha256": _sha256_file(contract_path),
        }
        layout_document["factory_common_zones"] = _factory_common_zone_metadata()
        ...
        try:
            ...
            _atomic_json(
                scene_dir / "quality_gates" / "reference_factory_layout_seed.json",
                receipt,
            )
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"reference-factory seed failed: {exc}", file=sys.stderr)
        return 1
    print(
        "reference-factory native layout passed: "
