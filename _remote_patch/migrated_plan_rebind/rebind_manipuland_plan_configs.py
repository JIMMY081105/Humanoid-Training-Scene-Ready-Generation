#!/usr/bin/env python3
"""Bind room plans to the exact ParaCloud recovery-worker Hydra configs."""
...
WORKERS = {
    ...
    "storage_room": ("paracloud_resume_storage_room", 401),
    "classroom_05": ("paracloud_resume_classroom_05", 405),
    "library": ("paracloud_resume_library", 410),
    "boys_toilet": ("resume_boys_toilet_rebound", 617),
    "classroom_06": ("paracloud_resume_classroom_06", 416),
    "classroom_02": ("paracloud_resume_classroom_02", 412),
    "classroom_03": ("paracloud_resume_classroom_03", 413),
...
def config_hash(repo: Path, run: Path, run_name: str, port_offset: int) -> str:
    ...
    try:
        # StatefulManipulandAgent receives only cfg.manipuland_agent. Bind the
        # same subtree, not the orchestration-only root config.
        resolved = OmegaConf.to_container(
            _load_cfg(args).manipuland_agent, resolve=True
        )
        return sha(canonical(resolved))
    finally:
        # The production worker resolves one config per process. This audit resolves
        # ten in one process, so clear the named resolvers between exact replays.
...
def main() -> int:
    ...
    for room_id, (run_name, port_offset) in WORKERS.items():
        expected = config_hash(repo, run, run_name, port_offset)
        ...
        with (states / ".manipuland_checkpoint.lock").open("a+b") as lock:
            ...
            old = document.get("config_sha256")
            output = dict(document)
            output["config_sha256"] = expected
            output["attestation"] = attest(output)
            args.backup_dir.mkdir(parents=True, exist_ok=True)
            backup = args.backup_dir / f"{room_id}.plan.{before}.json"
            ...
            records.append({
                "room_id": room_id, "run_name": run_name, "port_offset": port_offset,
                "old_config_sha256": old, "new_config_sha256": expected,
                "plan_before_sha256": before, "plan_after_sha256": sha(plan.read_bytes()),
            })
    receipt = {
        "schema_version": 1, "status": "pass",
        "operation": "rebind_manipuland_plans_to_exact_recovery_configs",
        "records": records, "quality_configuration_modified": False,
        "selection_content_modified": False,
    }
    receipt["attestation"] = attest(receipt)
    atomic_json(args.receipt, receipt, 0o600)
    print("MANIPULAND_CONFIG_REBIND_PASS", len(records))
    return 0
