...
- For the current reference-school run, Isaac is required: the terminal two-GPU allocation must launch the installed Isaac Sim runtime headlessly, import the exact published USD root, prove a valid default prim plus nonempty mesh, physics-scene, and collision inventories, advance the live physics timeline for at least 120 application updates, and then rehash the immutable package. OpenUSD parsing or an Isaac-compatibility rewrite alone is not an Isaac Sim execution pass. A terminal acceptance receipt must bind both this Isaac Sim result and the independent two-GPU Drake result.
