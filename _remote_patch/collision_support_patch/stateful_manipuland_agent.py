...
# This is the exact runtime immediately before the additive, collision-only shelf
# support repair.  It is accepted solely as a checkpoint/plan predecessor: all
# normal content, asset, receipt, and registry checks still run unchanged.
_COLLISION_SUPPORT_PREDECESSOR_RUNTIME_SHA256 = (
    "2b39c7bd572c1e0e607eb13cd8ed68f3b5a1e76db004f9f323d6c52da4ef9f1a"
)
...
class StatefulManipulandAgent(BaseStatefulAgent, BaseManipulandAgent):
    ...
    def _restore_completed_furniture_checkpoint_unlocked(
        ...
        *,
        ...
        # The support repair is additive and collision-only.  Permit durable
        # checkpoints made by its exact immediate predecessor while retaining all
        # receipt, scene-state, registry, and asset-byte verification below.
        is_collision_support_predecessor = (
            receipt.get("schema_version") == _FURNITURE_CHECKPOINT_SCHEMA
            and receipt.get("checkpoint_runtime_sha256")
            == _COLLISION_SUPPORT_PREDECESSOR_RUNTIME_SHA256
        )
        for key, expected in context.items():
            if key == "checkpoint_runtime_sha256" and (
                is_exact_predecessor
                or is_exact_filing_cabinet_predecessor
                or is_collision_support_predecessor
            ):
                continue
    ...
    @staticmethod
    def _requires_collision_support_shims(furniture: SceneObject) -> bool:
        """Return whether a storage-like asset needs explicit shelf collision.

        Support-surface extraction can correctly identify interior shelf planes on
        an ArtVIP/Artiverse asset even when its generated collision hull omits
        those planes.  This predicate is deliberately narrow: ordinary desks and
        tables retain their authored collision meshes unchanged.
        """
        description = f"{furniture.name} {furniture.description}".lower()
        return any(
            token in description
            for token in ("bookcase", "bookshelf", "shelf", "cubby", "rack", "cabinet")
        )

    @staticmethod
    def _collision_support_sdf(
        *,
        model_name: str,
        width: float,
        depth: float,
        ...
        """Build a visual-free static collision plate with deterministic bytes."""
        return (
            "<?xml version=\"1.0\"?>\n"
            "<sdf version=\"1.7\">\n"
            ...
            "    <static>true</static>\n"
            "    <link name=\"base_link\">\n"
            "      <collision name=\"shelf_support_collision\">\n"
            "        <geometry><box><size>"
            f"{width:.9g} {depth:.9g} {thickness:.9g}"
            "</size></box></geometry>\n"
            "        <surface><friction><ode><mu>1.0</mu><mu2>1.0</mu2>"
    ...
    @staticmethod
    def _collision_support_gltf() -> str:
        """Return an empty, valid glTF so the registry has no visual geometry."""
        return (
            '{"asset":{"version":"2.0","generator":"SceneSmith collision support"},'
            '"scene":0,"scenes":[{"nodes":[]}]}\n'
        )

    def _install_collision_support_shims(
        self,
        *,
        furniture: SceneObject,
        surfaces: list[SupportSurface],
    ) -> int:
        """Add durable, visual-free collision plates below extracted shelf planes.

        The source furniture SDF is intentionally never rewritten: it is an
        immutable, checksum-bound generated asset from an earlier checkpoint.
        Each plate is instead a new static manipuland asset, parented to the exact
        extracted support surface.  It is serialized, asset-registered, included
        in the final directive and therefore in both Drake and Isaac import.
        """
        if not self._requires_collision_support_shims(furniture):
            return 0
        if getattr(self, "asset_manager", None) is None:
            raise RuntimeError("Collision support repair requires an asset registry")

        root = (
            Path(self.scene.scene_dir)
            / "generated_assets"
            / "manipuland"
            / "sdf"
        )
        ...
        installed = 0
        thickness = 0.010
        owner = str(furniture.object_id)

        for surface in sorted(surfaces, key=lambda value: str(value.surface_id)):
            existing_instances = [
                candidate
                for candidate in self.scene.objects.values()
                if candidate.metadata.get("collision_support_owner") == owner
                and candidate.metadata.get("collision_support_surface")
                == str(surface.surface_id)
            ]
            if len(existing_instances) > 1:
                raise RuntimeError(
                    "Collision support repair found duplicate shim instances for "
                    f"{owner}/{surface.surface_id}"
                )
            if existing_instances:
                continue

            local_extent = surface.bounding_box_max - surface.bounding_box_min
            width = float(local_extent[0]) - 0.004
            depth = float(local_extent[1]) - 0.004
            if width <= 0.010 or depth <= 0.010:
                raise RuntimeError(
                    "Collision support repair received an invalid extracted shelf "
                    f"extent for {owner}/{surface.surface_id}: "
                    f"{local_extent.tolist()}"
                )
            center = (surface.bounding_box_min + surface.bounding_box_max) / 2.0
            safe_surface = re.sub(r"[^A-Za-z0-9_.-]", "_", str(surface.surface_id))
            safe_owner = re.sub(r"[^A-Za-z0-9_.-]", "_", owner)
            asset_name = f"collision_support_{safe_owner}_{safe_surface}"
            namespace = root / asset_name
            sdf_path = namespace / "collision_support_shim.sdf"
            gltf_path = namespace / "collision_support_shim.gltf"
            expected_sdf = self._collision_support_sdf(
                model_name=asset_name,
                width=width,
                depth=depth,
                thickness=thickness,
            )
            expected_gltf = self._collision_support_gltf()
            namespace.mkdir(parents=True, exist_ok=True)
            for path, expected in ((sdf_path, expected_sdf), (gltf_path, expected_gltf)):
                if path.exists():
                    if not path.is_file() or path.read_text(encoding="utf-8") != expected:
                        raise RuntimeError(
                            "Collision support asset bytes disagree with the exact "
                            f"surface contract: {path}"
                        )
                    continue
                with path.open("x", encoding="utf-8", newline="\n") as handle:
            ...
            asset_id = UniqueID(f"collision_support_asset_{safe_owner}_{safe_surface}")
            bbox_min = np.array([-width / 2.0, -depth / 2.0, -thickness / 2.0])
            bbox_max = np.array([width / 2.0, depth / 2.0, thickness / 2.0])
            asset_metadata: dict[str, str | float | bool] = {
                # RoomScene welds thin coverings; this static SDF then becomes a
                # fixed collision body without changing the visual scene.
                "asset_source": "thin_covering",
                "collision_support_shim": True,
                "collision_support_owner": owner,
                "collision_support_surface": str(surface.surface_id),
                "collision_support_schema": 1.0,
            }
            registered = registry.get(asset_id) if registry.exists(asset_id) else None
            if registered is None:
                registry.register(
                    SceneObject(
                        ...
                        name=asset_name,
                        description="Non-visual static collision support for an extracted shelf plane",
                ...
                registered.sdf_path is None
                or Path(registered.sdf_path).resolve(strict=True)
                != sdf_path.resolve(strict=True)
                or registered.metadata != asset_metadata
                or registered.bbox_min is None
                or registered.bbox_max is None
                or not np.array_equal(registered.bbox_min, bbox_min)
                or not np.array_equal(registered.bbox_max, bbox_max)
            ):
                raise RuntimeError(
                    "Collision support registry asset disagrees with the exact "
                    f"surface contract: {asset_id}"
                )

            object_id = self.scene.generate_unique_id(asset_name)
            self.scene.add_object(
                SceneObject(
                    ...
                    name=asset_name,
                    description="Non-visual static collision support for an extracted shelf plane",
                    transform=surface.to_world_pose(
                        center[:2], rotation_2d=0.0, z_offset=-thickness / 2.0
                    ),
                    geometry_path=gltf_path,
                    sdf_path=sdf_path,
                    metadata=asset_metadata,
                    ...
                    placement_info=PlacementInfo(
                        ...
                        position_2d=center[:2].copy(),
                        rotation_2d=0.0,
                        placement_method="collision_support_shim",
                    ),
                )
            )
            installed += 1
        ...
        if installed:
            console_logger.info(
                "Installed %d static collision support shim(s) for %s",
                installed,
                furniture.object_id,
            )
        return installed
        ...
        for furniture_index, furniture_selection in enumerate(furniture_data):
            ...
            with custom_span(
                ...
                try:
                    ...
                    self._install_collision_support_shims(
                        furniture=furniture,
                        surfaces=surfaces,
                    )
