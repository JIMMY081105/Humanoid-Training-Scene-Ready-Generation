...
def _accepted_render_evidence(
    ...
    *,
    ...
    """Bind the final score and the exact five copied critic views."""
    ...
    images = [directory / name for name in sorted(_ACCEPTED_IMAGE_NAMES)]
    allowed = {score.name, *_ACCEPTED_IMAGE_NAMES, *allowed_extra}
...
class StatefulManipulandAgent(BaseStatefulAgent, BaseManipulandAgent):
    ...
    def __init__(
        ...
        self._registry_scope_transaction: Any | None = None
    ...
    def _begin_furniture_scope(
        ...
        *,
        ...
        self._registry_scope_transaction = None
        ...
        if asset_manager is None:
            ...
            scene_states.mkdir(parents=True, exist_ok=True)
            transaction = asset_manager.registry.begin_scope_transaction(
                journal_path=(
                    scene_states / ".manipuland_registry_scope_transaction.json"
                ),
                context={
                    "furniture_id": str(furniture_id),
                    "furniture_index": furniture_index,
                },
                action_log_path=getattr(self.scene, "action_log_path", None),
            )
            self._registry_scope_transaction = transaction
            self._protected_registry_snapshot = transaction.baseline_snapshot
            self._protected_registry_sdf_paths = frozenset(
    ...
    def _commit_furniture_scope(self) -> None:
        """Commit registry additions after durable checkpoint publication."""
        transaction = getattr(self, "_registry_scope_transaction", None)
    ...
    def _restore_completed_furniture_checkpoint_unlocked(
        ...
        *,
        ...
        if asset_manager is not None:
            asset_manager.registry.commit_recovered_scope_transaction(
                journal_path=(
                    self.logger.output_dir
                    / "scene_states"
                    / ".manipuland_registry_scope_transaction.json"
                ),
                context={
                    "furniture_id": str(furniture_selection.furniture_id),
                    "furniture_index": furniture_index,
