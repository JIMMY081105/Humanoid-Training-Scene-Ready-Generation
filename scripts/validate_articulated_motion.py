#!/usr/bin/env python3
"""Prove that the three required school furniture roles really articulate.
...
def _parse_motion_joints(
    ...
    expected_type = str(requirement["joint_type"])
    ...
    for joint in (item for item in document.iter() if _local_name(item.tag) == "joint"):
        ...
        if joint_type != expected_type:
            raise ArticulatedMotionError(
                ...
                f"expected only {expected_type!r} movable joints"
            )
    ...
    if len(movable) < minimum:
        raise ArticulatedMotionError(
            f"{role} has {len(movable)} valid {expected_type} joints; at least {minimum} required"
        )
...
def evaluate(
    ...
    if set(ROLE_MOTION_REQUIREMENTS) != set(ARTICULATED_ROLE_RULES):
        raise ArticulatedMotionError(
            "Articulated-motion role table differs from the school room contract"
        )
    ...
    for role in sorted(ROLE_MOTION_REQUIREMENTS):
        ...
        if room_id != requirement["room_id"]:
            raise ArticulatedMotionError(
                f"{role} resolved to room {room_id!r}, expected {requirement['room_id']!r}"
            )
        room_dir = scene_dir / f"room_{room_id}"
