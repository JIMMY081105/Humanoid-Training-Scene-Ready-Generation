...
def _classify_mesh(record: dict[str, Any], room: dict[str, Any]) -> dict[str, Any]:
    ...
    if combined_span:
        ...
        geometric_wall = vertical and (
            (thin_x and on_x_boundary) or (thin_y and on_y_boundary)
        )
        semantic_wall = vertical and (exact_wall or _token_present(labels, "wall"))
...
def establish_cutaway(
    *,
    ...
    hidden_records = overhead + near_walls
...
def render_room(
    ...
    *,
    ...
    try:
        ...
        target = center
        horizontal_scale = max(width, depth) * 1.18
        diagonal_scale = (
        ...
        scene = bpy.context.scene
        for view_name, location, ortho_scale in views:
