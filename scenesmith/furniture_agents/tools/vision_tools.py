...
class VisionTools:
    ...
    def __init__(
        ...
        self._last_observed_scene_hash: str | None = None
        self._unchanged_observation_blocks = 0
        self.tools = self._create_tool_closures()

    def reset_observation_guard(self) -> None:
        """Allow one fresh observation at the start of a new agent turn.

        Repeated unchanged observations are blocked only within a single designer
        or critic run. A new user instruction may legitimately need the same scene
        rendered into that turn's context, especially when session trimming is on.
        """

        self._last_observed_scene_hash = None
        self._unchanged_observation_blocks = 0
    ...
    def _observe_scene_impl(self) -> list[ToolOutputImage | ToolOutputText]:
        ...
        if scene_hash == self._last_observed_scene_hash:
            self._unchanged_observation_blocks += 1
            console_logger.warning(
                "Blocked redundant observe_scene call for unchanged scene "
                "(count=%d, content_hash=%s)",
                self._unchanged_observation_blocks,
                scene_hash,
            )
            return [
                ToolOutputText(
                    text=(
                        "The scene has not changed since the previous observation, so "
                        "there is no new visual information and no images were rendered. "
                        "Do not call observe_scene again until a placement, move, removal, "
                        "rescale, or snap changes the scene. Continue with the next pending "
                        "TODO now; generate and place the required assets."
                    )
                )
            ]
        ...
        self._last_observed_scene_hash = scene_hash
        self._unchanged_observation_blocks = 0
