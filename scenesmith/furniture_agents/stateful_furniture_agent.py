...
class StatefulFurnitureAgent(BaseStatefulAgent, BaseFurnitureAgent):
    ...
    def __init__(
        ...
        self._designer_vision_tools: VisionTools | None = None
        self._critic_vision_tools: VisionTools | None = None
    ...
    def _create_critic_tools(self) -> list[FunctionTool]:
        ...
        """
        self._critic_vision_tools = VisionTools(
            scene=self.scene,
        ...
        return [
            self._critic_vision_tools.tools["observe_scene"],
    ...
    def _create_designer_tools(self) -> list[FunctionTool]:
        ...
        """
        self._designer_vision_tools = VisionTools(
            scene=self.scene,
            rendering_manager=self.rendering_manager,
            cfg=self.cfg,
            blender_server=self.blender_server,
        ...
        return [
            *self._designer_vision_tools.tools.values(),
    ...
    async def _request_initial_design_impl(self) -> str:
        if self._designer_vision_tools is not None:
            self._designer_vision_tools.reset_observation_guard()
        return await super()._request_initial_design_impl()

    async def _request_design_change_impl(self, instruction: str) -> str:
        if self._designer_vision_tools is not None:
            self._designer_vision_tools.reset_observation_guard()
        return await super()._request_design_change_impl(instruction)

    async def _request_critique_impl(self, update_checkpoint: bool = True) -> str:
        if self._critic_vision_tools is not None:
            self._critic_vision_tools.reset_observation_guard()
        return await super()._request_critique_impl(update_checkpoint=update_checkpoint)
