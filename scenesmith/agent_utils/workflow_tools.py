...
class WorkflowTools:
        ...
        def designer_todo_manager(action: str, task: str | None = None) -> str:
            ...
            Args:
                action: What to do - 'add' for new tasks, 'complete' to mark the
                       next pending task done, 'get_next' to see what to do next, or
    ...
    def _designer_todo_manager_impl(self, action: str, task: str | None = None) -> str:
        ...
        if action == "add":
            ...
            # ``get_next`` is FIFO, so ``complete`` must advance that same task.
            # Reversing here used to mark the newest (often final-validation)
            # TODO complete while the oldest planning/generation task remained
            # pending, allowing agents to skip required work.
