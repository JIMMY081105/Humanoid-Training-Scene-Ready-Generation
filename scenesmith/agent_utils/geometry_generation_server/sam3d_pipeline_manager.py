...
def generate_mask(
    ...
    # SAM3's fused ViT MLP returns bfloat16 activations between its two
    # float32 Linear layers.  The official SAM3 inference examples therefore
    # run both image and prompt encoding under CUDA bfloat16 autocast.  Keep
    # the complete processor interaction in one context because the image
    # features produced by set_image are consumed by set_text_prompt.
    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        inference_state = sam3_processor.set_image(image)

        # Generate segmentation. set_text_prompt returns updated
        # inference_state with masks/scores.
    ...
    if torch.is_tensor(scores):
        # SAM3 produces bfloat16 confidence scores under its required
        # autocast context, while NumPy has no native bfloat16 conversion.
